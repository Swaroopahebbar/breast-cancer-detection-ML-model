...
TrainFiles = dir('Training\');
Train_Number=0;
for i = 1:size(TrainFiles,1)
    if not(strcmp(TrainFiles(i).name,'.')|strcmp(TrainFiles(i).name,'..') | strcmp(TrainFiles(i).name,'Thumbs.db'))
        Train_Number = Train_Number + 1; % Number of all images in the training database
        name{Train_Number}=TrainFiles(i).name; %#ok<SAGROW>
         label{Train_Number}=name{Train_Number}; %#ok<SAGROW>
        TrainFiles1{Train_Number} = strcat('Training','\',name{Train_Number}); %#ok<SAGROW>
    end
end
...
for i1 = 1:length(TrainFiles1)
    ...
    if not(strcmp(TrainFiles2(j1).name,'.')|strcmp(TrainFiles2(j1).name,'..') | strcmp(TrainFiles2(j1).name,'Thumbs.db'))
       ...
       label{Train_Number1}=n2;
       label1{Train_Number1}=i1;
    end
    end
