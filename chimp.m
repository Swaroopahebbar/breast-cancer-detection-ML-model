function [Attacker_score,Attacker_pos,Convergence_curve]=Chimp(trainfeature,label1,testfeature,testlabel1)
SearchAgents_no=2;Max_iter=10;lb=1;ub=30;dim=1;
...
while l<Max_iter
    ...
    for i=1:size(Positions,1)  
       ...
       % Return back the search agents that go beyond the boundaries of the search space
        ...
        fitness=fobj(Positions(i,:),trainfeature,label1,testfeature,testlabel1);
...
%% % Please note that to choose various Chaotic maps you should use the related Chaotic maps strategies
            m=chaos(3,1,1); %  (5)
            D_Attacker=abs(C1*Attacker_pos(j)-m*Positions(i,j)); %  (6)
            X1=Attacker_pos(j)-A1*D_Attacker; %  (7)
                       
            A2=2*f*r21-f; %  (3)
            C2=2*r22; %  (4)
            
                   
            D_Barrier=abs(C2*Barrier_pos(j)-m*Positions(i,j)); %  (6)
            X2=Barrier_pos(j)-A2*D_Barrier; %  (7)     
            ...
            D_Driver=abs(C3*Chaser_pos(j)-m*Positions(i,j)); %  (6)
            X3=Chaser_pos(j)-A3*D_Driver; %  (7)      
            
            A4=2*f*r41-f; %  (3)
            C4=2*r42; %  (4)
            
            D_Driver=abs(C4*Driver_pos(j)-m*Positions(i,j)); %  (6)
            X4=Chaser_pos(j)-A4*D_Driver; %  (7)       
            
            Positions(i,j)=(X1+X2+X3+X4)/4;%  (8)
